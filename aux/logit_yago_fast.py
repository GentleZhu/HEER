...
with open(input_label_train, "r") as f_in:
    ...
    for line in f_in:
        ...
        if line[0] not in embedding_dict or line[1] not in embedding_dict:
            continue
        ...
        if '-1' not in edge:
            ...
            if edge_neg not in feature_train_dic:
                feature_train_dic[edge_neg]={}
                feature_train_dic[edge_neg]['tuple']=[]
                feature_train_dic[edge_neg]['yvalue']=[]    
            feature_train_dic[edge_neg]['tuple'].append(np.multiply(node_1,node_2))
            feature_train_dic[edge_neg]['yvalue'].append(y_value)
        else:
            edge_pos=edge[0:-2]
            feature_train_dic[edge_pos]['tuple'].append(np.multiply(node_1,node_2))
            feature_train_dic[edge_pos]['yvalue'].append(y_value)
            feature_train_dic[edge]['tuple'].append(np.multiply(node_1,node_2))
            feature_train_dic[edge]['yvalue'].append(y_value)
        count+=1
        if count==50000:
...
for edge in feature_train_dic.keys():
    ...
    logit_model = LogisticRegression(solver="sag",max_iter=100)  
...
#hadamard-prod emb(node_2) -- where node_1 and node_2 are the two nodes on a line of file *2*
    proba_test = feature_train_dic[edge]['model'].predict_proba(feature_test_dic[edge]['Xtest'])
    #print(proba_test[:,1])
