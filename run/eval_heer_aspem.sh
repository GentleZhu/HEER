...
# files
score_file="$root_dir"/intermediate_data/heer_"$network"_aspect_"$aspects"_"$epoch"_"$operator"_"$map"_"$more_param".txt
...
output_file="$root_dir"/output/out_heer_"$network"_aspect_"$aspects"_"$epoch"_"$operator"_"$map"_"$more_param"_"$time_start".txt
...
if [ "$per_epoch_eval_time_start" != "null" ]; then
  tail -n 1 "$output_file" | awk '{print $NF}' >> "$root_dir"/output/mrr_micro_heer_"$network"_aspect_"$aspects"_"$operator"_"$map"_"$more_param"_"$per_epoch_eval_time_start".txt
  tail -n 2 "$output_file" | head -n 1 | awk '{print $NF}' >> "$root_dir"/output/mrr_macro_heer_"$network"_aspect_"$aspects"_"$operator"_"$map"_"$more_param"_"$per_epoch_eval_time_start".txt
fi
