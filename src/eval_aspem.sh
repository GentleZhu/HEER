...
aspect_number=${8:-0}  # must specify, expect default to fail
...
fast_eval_file="$root_dir"/input_data/"$network"_aspect_"$aspect_number"_eval_fast.txt
if [ -f "$fast_eval_file" ]; then
	...
	eval_file="$root_dir"/input_data/"$network"_aspect_"$aspect_number"_eval.txt
	fast=0
fi

per_type_cur_model_dir="$root_dir"/intermediate_data/per_type_temp/"$network"_aspect_"$aspect_number"_"$operator"_"$map"_"$more_param"_"$time_start"
...
until [  $curr_step -gt $((epoch - 1)) ]; do
	echo $curr_step
	python2 "$root_dir"/src/pred.py --iter=$curr_step --batch-size=128 --dimensions=128  --graph-name="$network"_aspect_"$aspect_number" --data-dir="$root_dir"/intermediate_data/ --model-dir="$root_dir"/intermediate_data/model/ \
