...
if [ ! -e  "$root_dir"/intermediate_data/"$network"_aspect_"$aspect_number""$SUFFIX" ]; then
	...
	python ./src/main.py --input="$root_dir"/input_data/"$network"_aspect_"$aspect_number".hin --build-graph=True \
		--graph-name="$network"_aspect_"$aspect_number" --data-dir="$root_dir"/intermediate_data/
fi
echo ${red}===HEER Training===${reset}
python ./src/main.py --iter=$2 --batch-size=128 --dimensions=128  --graph-name="$network"_aspect_"$aspect_number" --data-dir="$root_dir"/intermediate_data/ --model-dir="$root_dir"/intermediate_data/model/ \
